...
int setup_map_system(char *requests[], int size)
{
  if (size > MAX_NR_MAPS) {
 ...
 */
static int _get_map_fd(char *map_name)
{
...
static int _get_map_fd_and_idx(char *map_name, int *idx)
{
  for (int i = 0; i < MAX_NR_MAPS; i++) {
    if (map_names[i] == NULL) {
      // List finished and did not found the FD of the map
...
inline void * ubpf_map_lookup_elem_kern(char *map_name, const void *key_ptr)
{
  int idx;
...
int ubpf_map_update_elem_kern(char *map_name, const void *key_ptr,
    void *value, int flag)
{

...
int bmc_tx_filter_main(struct __sk_buff *skb)
{
	...
	if (skb->len > BMC_MAX_CACHE_DATA_SIZE + sizeof(struct ethhdr) +
	...
	/* 		.p_end = 0, */
	/* 	}; */
	/* 	bpf_perf_event_output(skb, &debug_map, 0, &data, sizeof(data)); */
	/* } */

	if (ip->protocol != IPPROTO_UDP)
		return TC_ACT_OK;

	/* /1* if(payload + 7 < data_end) *1/ */
	/* { */
	/* 	// DEBUG */
	/* 	struct debug_info data = { */
	/* 		.type = udp_detail, */
	/* 		/1* .sval = bpf_ntohs(sport), *1/ */
	/* 		.sval = 0, */
	/* 		.cmd = {}, */
	/* 		.p_start = (u32)(((void*)data_end) - ((void *)payload)), */
	/* 		.p_end = 0, */
	/* 	}; */
	/* 	/1* for (int i = 0; i < 6; i++) { *1/ */
	/* 	/1* 	data.cmd[i] = payload[i]; *1/ */
	/* 	/1* } *1/ */
	/* 	/1* data.cmd[6] = '\0'; *1/ */
	/* 	bpf_perf_event_output(skb, &debug_map, 0, &data, sizeof(data)); */
	/* } */
	...
	if (sport == bpf_htons(DST_PORT) && payload+5+1 <= data_end &&
			payload[0] == 'V' && payload[1] == 'A' && payload[2] ==
			'L' && payload[3] == 'U' && payload[4] == 'E' &&
			payload[5] == ' ') { // if this is a GET reply
